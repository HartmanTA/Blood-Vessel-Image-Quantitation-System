...
class findvesselLogic(ScriptedLoadableModuleLogic):
    ...
    def batchSaE(self, vSet, centroid):
        saeSet = []
        for v in vSet:
            saeSet.append(self.startAndEnd(v, centroid))
        return saeSet
    ...
    def process(self,
        ...
        #takes volume selected in 3d slicer and puts in a numpy array
        volumeArray = slicer.util.arrayFromVolume(inputVolume)

        #perform processing
        centroid = self.centroidFinder(volumeArray)
        vessels = self.vesselFinder(volumeArray)
        batch = self.batchSaE(vessels, centroid)
